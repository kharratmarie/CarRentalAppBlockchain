...
modifier onlyCarOwner(uint carId) {
    ...
    event UserAdded(uint256 id, string name, string email, string phone, string cin);
    ...
    function registerUser(
        ...
        userCount++;
        users[msg.sender] = User(userCount, name, email, phone, cin);
        userAddresses.push(msg.sender); // Ajouter l'adresse de l'utilisateur au tableau

        emit UserAdded(userCount, name, email, phone, cin);
    }
...
//     UserCount ++ ; 
//     users[msg.sender ] = User(UserCount, name, email, phone, cin);
//     emit UserAddedDebug(UserCount, name, email, phone, cin);
